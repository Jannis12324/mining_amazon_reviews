...
(da, column_names) = bi_dict_analysis(df,"demographics_dict.xlsx")
rdf= da.copy()
grdf= rdf.groupby(["kindle"]).sum()
count = rdf.groupby(["kindle"]).count()
count.drop(count.columns[1:],axis=1,inplace=True)
grdf.drop(grdf.columns[0:2],axis=1,inplace=True)
results = grdf
latex = grdf.T
...
#Create a wordcloud
#Dataframe with the desired reviews:
eld = df[df.Elderly ==1]
adu = df[df.Adults ==1]
#Create stopword list
stopwords = ["one","read"]
get_ipython().run_line_magic('run', 'func_word_cloud.ipynb')
make_wordcloud(eld, stopwords, "elderly_wordcloud.png")
